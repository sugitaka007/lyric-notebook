...
const tabs = [["lyrics", "歌詞"], ["ideas", "アイデア"], ["photos", "写真"], ["recording", "録音"], ["audio", "音声"], ["sketch", "スケッチ"]] as const;
export type SongTab = typeof tabs[number][0];
const stages: SongStage[] = ["種", "制作中", "推敲中", "完成", "保留"];

export function SongEditor(props: TabProps & { patchSong(patch: Partial<Song>): void; saveState: SaveState; initialTab: SongTab; focusTitle: boolean; onBack(): void; onDelete(song: Song): void }) {
  ...
  return (
    <main className="editor-shell">
      <header className="editor-header"><button className="back-button" onClick={props.onBack} aria-label="曲一覧へ戻る">‹</button><div className="editor-title"><input ref={titleInput} aria-label="曲名" value={props.song.title} onChange={(event) => props.patchSong({ title: event.target.value })} placeholder="曲名" /><span className={`save-state ${props.saveState}`}>{props.saveState === "saving" ? "保存中" : props.saveState === "error" ? "保存失敗" : "保存済み"}</span></div><button className="menu-button" onClick={() => setInfoOpen(true)} aria-label="曲情報">•••</button></header>
      <div className="editor-content">
        {tab === "lyrics" && <LyricsTab {...common} />}
        {tab === "ideas" && <IdeasTab {...common} />}
        {tab === "photos" && <PhotosTab {...common} />}
        {tab === "recording" && <RecordingTab {...common} />}
        {tab === "audio" && <AudioTab {...common} />}
        {tab === "sketch" && <SketchTab {...common} />}
      </div>
      <nav className="bottom-tabs" aria-label="曲の編集タブ">{tabs.map(([id, label]) => <button key={id} className={tab === id ? "active" : ""} onClick={() => setTab(id)}><span>{label}</span></button>)}</nav>

      {infoOpen && <div className="modal-backdrop"><div className="modal song-info"><div className="modal-title"><h2>曲情報</h2><button onClick={() => setInfoOpen(false)} aria-label="閉じる">×</button></div><label>曲名<input value={props.song.title} onChange={(event) => props.patchSong({ title: event.target.value })} /></label><label>曲の概要<textarea value={props.song.summary} onChange={(event) => props.patchSong({ summary: event.target.value })} /></label><label>制作段階<select value={props.song.stage} onChange={(event) => props.patchSong({ stage: event.target.value as SongStage })}>{stages.map((stage) => <option key={stage}>{stage}</option>)}</select></label><label>タグ<input value={props.song.tags.join("、")} onChange={(event) => props.patchSong({ tags: event.target.value.split(/[、,]/).map((tag) => tag.trim()).filter(Boolean) })} placeholder="、で区切る" /></label>{legacyFields.length > 0 && <details className="legacy-info"><summary>以前の曲情報</summary><dl>{legacyFields.map(([label, value]) => <div key={label}><dt>{label}</dt><dd>{value}</dd></div>)}</dl></details>}<button className="danger full" onClick={confirmDelete}>曲を削除</button></div></div>}
    </main>
  );
}
