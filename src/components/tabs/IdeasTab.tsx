...
export function IdeasTab({ song, workspace, setWorkspace, queueSave, notify }: TabProps) {
  ...
  async function addIdea() {
    ...
    const stamp = now();
    const idea: Idea = { id: uid(), songId: song.id, text: draft.trim(), category: category || undefined, pinned: false, assetIds: [], createdAt: stamp, updatedAt: stamp };
    try { await db.ideas.add(idea); setWorkspace((data) => ({ ...data, ideas: [idea, ...data.ideas] })); setDraft(""); setCategory(""); }
  ...
  return (
    <section className="tab-page ideas-page"><div className="tab-heading"><h1>アイデア</h1></div><div className="idea-composer"><textarea value={draft} onChange={(event) => setDraft(event.target.value)} placeholder="" aria-label="新しいアイデア" /><div><select aria-label="分類（任意）" value={category} onChange={(event) => setCategory(event.target.value as IdeaCategory | "")}><option value="">分類なし</option>{categories.map((item) => <option key={item}>{item}</option>)}</select><button className="primary" disabled={!draft.trim()} onClick={() => void addIdea()}>追加</button></div></div>
      <div className="idea-list">{ideas.map((idea) => <article className={idea.pinned ? "idea-card pinned" : "idea-card"} key={idea.id}><textarea aria-label="アイデアの内容" value={idea.text} onChange={(event) => patchIdea(idea, { text: event.target.value })} /><div className="idea-images">{idea.assetIds.map((id) => { const asset = workspace.media.find((item) => item.id === id); return asset?.blob ? <BlobImage key={id} blob={asset.blob} alt={asset.name} /> : null; })}</div><div className="idea-controls"><select aria-label="分類" value={idea.category ?? ""} onChange={(event) => patchIdea(idea, { category: (event.target.value || undefined) as IdeaCategory | undefined })}><option value="">分類なし</option>{categories.map((item) => <option key={item}>{item}</option>)}</select><button onClick={() => patchIdea(idea, { pinned: !idea.pinned })}>{idea.pinned ? "固定解除" : "固定"}</button><label className="inline-file"><input type="file" accept="image/*" onChange={(event) => void addImage(idea, event.target.files?.[0])} />画像</label><button className="danger-text" onClick={() => void removeIdea(idea)}>削除</button></div></article>)}</div>
      {ideas.length === 0 && <p className="plain-empty">アイデアはありません。</p>}
    </section>
  );
}
