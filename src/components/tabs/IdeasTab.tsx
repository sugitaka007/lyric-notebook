...
import { db, markSongUsed, now, removeInboxUse, uid } from "../../db";
...
export function IdeasTab({ song, workspace, setWorkspace, queueSave, settings, notify }: TabProps) {
  ...
  async function removeIdea(idea: Idea) {
    ...
    if (idea.sourceInboxId) await removeInboxUse(idea); else await db.ideas.delete(idea.id);
  ...
  async function addImage(idea: Idea, file?: File) {
    ...
    try {
      ...
      await db.transaction("rw", [db.media, db.ideas], async () => { await db.media.add(asset); await db.ideas.put(updated); }); await markSongUsed(song.id);
  ...
  async function addUrl() {
    try {
      ...
      const asset: MediaAsset = { id: uid(), songId: song.id, kind: "url", name: urlName.trim() || parsed.hostname, note: "", mimeType: "text/uri-list", size: urlText.length, url: parsed.toString(), links: [], createdAt: stamp, updatedAt: stamp };
      await db.media.add(asset); await markSongUsed(song.id); setWorkspace((data) => ({ ...data, media: [asset, ...data.media] })); setUrlText(""); setUrlName("");
  ...
  return (
    <section className="tab-page ideas-page"><div className="tab-heading"><h1>アイデア</h1></div>
      ...
      <div className="idea-list">{ideas.map((idea) => <details className="idea-card" key={idea.id}><summary><span>{idea.category ?? "分類なし"}</span><b>{idea.text}</b></summary><textarea rows={3} aria-label="アイデアの内容" value={idea.text} onChange={(event) => patchIdea(idea, { text: event.target.value })} /><div className="idea-images">{idea.assetIds.map((id) => { const asset = workspace.media.find((item) => item.id === id); return asset?.blob ? <BlobImage key={id} blob={asset.blob} alt={asset.name} /> : null; })}</div><div className="idea-controls"><select aria-label="分類" value={idea.category ?? ""} onChange={(event) => patchIdea(idea, { category: (event.target.value || undefined) as IdeaCategory | undefined })}><option value="">分類なし</option>{categories.map((item) => <option key={item}>{item}</option>)}</select>{idea.sourceInboxId && <button onClick={() => void db.inbox.get(idea.sourceInboxId!).then((source) => setSourceText(source?.text ?? "元メモが見つかりません。"))}>元メモを確認</button>}<button onClick={() => setCopyIdeaId(idea.id)}>GPT用にコピー</button><label className="inline-file"><input type="file" accept="image/*" onChange={(event) => void addImage(idea, event.target.files?.[0])} />画像</label><button className="danger-text" onClick={() => void removeIdea(idea)}>{idea.sourceInboxId ? "曲から外す" : "削除"}</button></div></details>)}</div>
      {ideas.length === 0 && <p className="plain-empty compact-empty">アイデアはありません。</p>}
      <details className="reference-panel"><summary>参考リンク {links.length}</summary><div className="url-adder"><input value={urlName} onChange={(event) => setUrlName(event.target.value)} placeholder="表示名（任意）" /><input inputMode="url" value={urlText} onChange={(event) => setUrlText(event.target.value)} placeholder="URL" /><button disabled={!urlText} onClick={() => void addUrl()}>追加</button></div>{links.map((asset) => <div className="reference-link" key={asset.id}><a href={asset.url} target="_blank" rel="noreferrer">{asset.name}</a><button className="danger-text" onClick={() => void removeLink(asset)}>削除</button></div>)}</details>
      {sourceText !== undefined && <div className="modal-backdrop"><div className="modal"><div className="modal-title"><h2>元メモ</h2><button onClick={() => setSourceText(undefined)} aria-label="閉じる">×</button></div><p className="source-memo-text">{sourceText}</p></div></div>}
