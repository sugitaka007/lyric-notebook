...
export function LyricsTab({ song, workspace, setWorkspace, queueSave, notify }: TabProps) {
  ...
  return <section className="tab-page lyrics-page"><div className="tab-heading"><h2>歌詞</h2><select aria-label="歌詞セクションを追加" value="" onChange={(e) => { if (e.target.value) addSection(e.target.value); }}><option value="">＋ セクション</option>{sectionPresets.map((x) => <option key={x}>{x}</option>)}<option>自由セクション</option></select></div>
