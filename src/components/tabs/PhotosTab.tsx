...
export function PhotosTab({ song, workspace, setWorkspace, queueSave, notify }: TabProps) {
  ...
  async function addImages(files?: FileList | null) {
    ...
    for (const file of Array.from(files)) {
      try { const blob = await compressImage(file); const stamp = now(); const asset: MediaAsset = { id: uid(), songId: song.id, kind: "image", name: file.name, note: "", mimeType: blob.type, blob, size: blob.size, links: [], createdAt: stamp, updatedAt: stamp }; await db.media.add(asset); setWorkspace((data) => ({ ...data, media: [asset, ...data.media] })); }
  ...
  return <section className="tab-page"><div className="tab-heading"><h1>写真</h1></div><div className="split-actions"><label><input type="file" accept="image/*" multiple onChange={(event) => void addImages(event.target.files)} />写真から追加</label><label><input type="file" accept="image/*" capture="environment" onChange={(event) => void addImages(event.target.files)} />カメラで撮影</label></div><div className="material-list">{photos.map((asset) => <AssetCard key={asset.id} asset={asset} onPatch={(values) => patch(asset, values)} onDelete={() => void remove(asset)} />)}</div>{photos.length === 0 && <p className="plain-empty compact-empty">写真はありません。</p>}</section>;
}
