...
function CanvasEditor({ record, lines, scenes, onUpdate, onDelete, notify }: { record: SketchRecord; lines: TabProps["workspace"]["lines"]; scenes: TabProps["workspace"]["scenes"]; onUpdate(next: SketchRecord): void; onDelete(): void; notify(message: string): void }) {
  ...
  return <article className="canvas-editor"><div className="sketch-name-row"><input value={draft.name} onChange={(e) => patch({ name: e.target.value })} aria-label="スケッチ名" /><button className="danger-text" onClick={onDelete}>削除</button></div><div className="canvas-toolbar"><div className="tool-pair"><button className={tool === "pen" ? "active" : ""} onClick={() => setTool("pen")}>ペン</button><button className={tool === "eraser" ? "active" : ""} onClick={() => setTool("eraser")}>消しゴム</button></div><label className="color-control"><input type="color" value={color} onChange={(e) => setColor(e.target.value)} /><span>色</span></label><label className="width-control"><span>太さ</span><input type="range" min="1" max="28" value={width} onChange={(e) => setWidth(Number(e.target.value))} /></label><button onClick={undo} disabled={!draft.strokes.length}>↶</button><button onClick={redoStroke} disabled={!redo.length}>↷</button><button onClick={() => { if (window.confirm("描いた線をすべて消しますか？")) setDraft((item) => ({ ...item, strokes: [] })); }}>全消去</button></div><div className="aspect-row"><div className="segmented small">{(["16:9", "9:16", "1:1"] as AspectRatio[]).map((aspect) => <button key={aspect} className={draft.aspect === aspect ? "active" : ""} onClick={() => patch({ aspect })}>{aspect}</button>)}</div><label className="small-file"><input type="file" accept="image/*" onChange={(e) => setUnderlay(e.target.files?.[0])} />{draft.underlayBlob ? "下敷きを変更" : "＋ 画像を下敷き"}</label></div><div className="canvas-wrap" style={{ aspectRatio: ratio }}><canvas ref={canvasRef} onPointerDown={pointerDown} onPointerMove={pointerMove} onPointerUp={pointerUp} onPointerCancel={pointerUp} /></div><div className="sketch-links"><label>関連歌詞<select value={draft.relatedLyricId ?? ""} onChange={(e) => patch({ relatedLyricId: e.target.value || undefined })}><option value="">未選択</option>{lines.filter((x) => x.text).map((line) => <option key={line.id} value={line.id}>{line.text}</option>)}</select></label><label>関連MV場面<select value={draft.relatedSceneId ?? ""} onChange={(e) => patch({ relatedSceneId: e.target.value || undefined })}><option value="">未選択</option>{scenes.map((scene) => <option key={scene.id} value={scene.id}>{scene.name}</option>)}</select></label></div><div className="canvas-actions"><button onClick={() => save(false)}>端末内へ保存</button><button className="primary" onClick={() => save(true)}>PNGを書き出す</button></div></article>;
}
