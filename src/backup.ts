...
import type { AssociationCard, InboxItem, LyricLine, LyricSection, MediaAsset, MVScene, SketchRecord, Song } from "./types";

export const BACKUP_FORMAT_VERSION = 1;

interface BackupData {
  songs: Song[]; sections: LyricSection[]; lyricLines: LyricLine[]; associations: AssociationCard[];
  mvScenes: MVScene[]; sketches: Array<Omit<SketchRecord, "previewBlob" | "underlayBlob"> & { previewFile?: string; underlayFile?: string }>;
  media: Array<Omit<MediaAsset, "blob"> & { blobFile?: string }>; inbox: InboxItem[];
}

interface BackupManifest {
  app: "yohaku-lyric-notebook"; formatVersion: number; createdAt: string;
  counts: Record<keyof Pick<BackupData, "songs" | "sections" | "lyricLines" | "associations" | "mvScenes" | "sketches" | "media" | "inbox">, number>;
}
...
export async function createBackupBlob() {
  const [songs, sections, lyricLines, associations, mvScenes, sketchesRaw, mediaRaw, inbox] = await Promise.all([
    db.songs.toArray(), db.sections.toArray(), db.lyricLines.toArray(), db.associations.toArray(),
  ...
  const data: BackupData = { songs, sections, lyricLines, associations, mvScenes, sketches, media, inbox };
  const createdAt = now();
  const manifest: BackupManifest = {
    app: "yohaku-lyric-notebook", formatVersion: BACKUP_FORMAT_VERSION, createdAt,
    counts: { songs: songs.length, sections: sections.length, lyricLines: lyricLines.length, associations: associations.length, mvScenes: mvScenes.length, sketches: sketches.length, media: media.length, inbox: inbox.length },
  };
...
async function parseBackup(file: Blob) {
  ...
  let manifest: BackupManifest; let raw: BackupData;
  ...
  if (manifest.formatVersion !== BACKUP_FORMAT_VERSION) throw new Error(`未対応のバックアップ形式（v${manifest.formatVersion}）です。`);
  for (const key of ["songs", "sections", "lyricLines", "associations", "mvScenes", "sketches", "media", "inbox"] as const) {
    assertArray(raw[key], key); if (raw[key].length !== manifest.counts[key]) throw new Error(`${key} の件数がmanifestと一致しません。`);
  }
  const media: MediaAsset[] = [];
  for (const item of raw.media) {
    let blob: Blob | undefined;
    if (item.blobFile) { const entry = zip.file(item.blobFile); if (!entry) throw new Error(`素材ファイル ${item.blobFile} がありません。`); blob = await entry.async("blob"); }
    const { blobFile: _blobFile, ...record } = item; void _blobFile; media.push({ ...record, blob });
  }
  const sketches: SketchRecord[] = [];
  for (const item of raw.sketches) {
    let previewBlob: Blob | undefined; let underlayBlob: Blob | undefined;
    ...
    const { previewFile: _previewFile, underlayFile: _underlayFile, ...record } = item; void _previewFile; void _underlayFile; sketches.push({ ...record, previewBlob, underlayBlob });
  }
  return { manifest, data: { ...raw, media, sketches } };
}

function remapForMerge(data: Awaited<ReturnType<typeof parseBackup>>["data"]) {
  const maps = {
    song: new Map(data.songs.map((x) => [x.id, uid()])), section: new Map(data.sections.map((x) => [x.id, uid()])),
    lyric: new Map(data.lyricLines.map((x) => [x.id, uid()])), association: new Map(data.associations.map((x) => [x.id, uid()])),
    scene: new Map(data.mvScenes.map((x) => [x.id, uid()])), sketch: new Map(data.sketches.map((x) => [x.id, uid()])),
    media: new Map(data.media.map((x) => [x.id, uid()])), inbox: new Map(data.inbox.map((x) => [x.id, uid()])),
  };
  ...
  return {
    ...
    inbox: data.inbox.map((x) => ({ ...x, id: maps.inbox.get(x.id)!, assetId: x.assetId ? maps.media.get(x.assetId) : undefined })),
  };
}
...
export async function restoreBackup(file: Blob, mode: "merge" | "replace") {
  const parsed = await parseBackup(file); const data = mode === "merge" ? remapForMerge(parsed.data) : parsed.data;
  await db.transaction("rw", db.tables, async () => {
    if (mode === "replace") await Promise.all(db.tables.map((table) => table.clear()));
    await db.songs.bulkPut(data.songs); await db.sections.bulkPut(data.sections); await db.lyricLines.bulkPut(data.lyricLines);
    await db.associations.bulkPut(data.associations); await db.mvScenes.bulkPut(data.mvScenes); await db.sketches.bulkPut(data.sketches);
    await db.media.bulkPut(data.media); await db.inbox.bulkPut(data.inbox);
